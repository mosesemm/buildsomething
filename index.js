...
function tasksToRows(_tasks) {
    return _tasks.map(t => {
        return `<tr style="${rowStroke(t)}"><td>${t.description}</td><td><a data-task="${btoa(JSON.stringify(t))}" href="#" id="viewTask" onClick="editOrViewTask(this.dataset.task)">Edit/View</a> 
        <a href="#" style="color: red" data-task="${btoa(JSON.stringify(t))}" onClick="event.preventDefault();deleteTask(this.dataset.task)">Delete</a></td></tr>`;
    });
}
